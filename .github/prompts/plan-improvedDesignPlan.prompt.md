...
2. Phase 2 - Shared surface consolidation (estimated 6-10 hours)
   ...
   - Dependencies: none.
   - Parallelism: Can run in parallel with Phase 1.
...
3. Phase 3 - Reliability fixes (estimated 10-14 hours)
   ...
   - Dependencies: none, but Phase 4 depends on this phase for trustworthy QA outcomes.
...
4. Phase 4 - Route-level brand alignment (estimated 12-18 hours)
   ...
   - Dependencies: depends on Phases 1-3.   - Parallelism: Universities/Compare/Onboarding can run in parallel subtracks after dependencies are complete.
...
5. Phase 5 - State-completeness and accessibility pass (estimated 4-6 hours)
   ...
   - Dependencies: depends on Phase 4.
...
6. Phase 6 - Regression verification and release gate (estimated 3-5 hours)
   ...
   - Dependencies: depends on Phase 5.

**Relevant files**
- frontend/src/app/globals.css - source of truth for brand token and animation utilities; enforce canonical palette mapping.
- frontend/src/app/globals.css - source of truth for brand token and animation utilities; enforce canonical palette mapping.
- frontend/src/app/layout.tsx - global shell and shared surfaces composition baseline.
- frontend/src/components/header.tsx - normalize nav action hierarchy and spacing across auth states.
- frontend/src/components/footer.tsx - unify footer appearance and spacing.
- frontend/src/components/ui/ - central button/card/input/select primitives to standardize CTAs and surfaces.
- frontend/src/app/universities/page.tsx - loading/error/empty states and listing UI consistency.
- backend/src/modules/universities/universities.service.ts - likely filter query parameter indexing issue causing hanging searches.
- frontend/src/app/compare/page.tsx - compare empty/loading UX and auth-compatible behavior.
- frontend/src/middleware.ts - enforce auth-required policy for /compare consistently.
- frontend/src/app/sign-in/[[...sign-in]]/page.tsx - remove deprecated Clerk prop usage.
- frontend/src/app/sign-up/[[...sign-up]]/page.tsx - remove deprecated Clerk prop usage.
- frontend/src/app/universities/[id]/page.tsx - resilient media/logo fallback behavior.
- frontend/src/app/dashboard/saved/ - sparse/empty state brand alignment and CTA guidance.
- frontend/src/app/dashboard/applications/ - sparse/empty state brand alignment and status semantics.
- frontend/src/app/onboarding/ - progression and CTA brand consistency.
- frontend/src/app/privacy/ and frontend/src/app/terms/ - legal page spacing/heading system consistency.**Verification**
